...
func headerHasNonEmptyValue(header http.Header, key string) bool {
	if len(header) == 0 {
		return false
	}
...
func extractOpenAICodexProbeSnapshot(resp *http.Response) (map[string]any, *time.Time, error) {
	...
	if resp.StatusCode == http.StatusForbidden {
		updates["probe_last_status"] = http.StatusForbidden
		if headerHasNonEmptyValue(resp.Header, "cf-mitigated") {
			updates["probe_cf_blocked"] = true
			updates["privacy_mode"] = PrivacyModeCFBlocked
		}
	}
	if snapshot := ParseCodexRateLimitHeaders(resp.Header); snapshot != nil {
		baseTime := time.Now()
		maps.Copy(updates, buildCodexUsageExtraUpdates(snapshot, baseTime))
		resetAt := codexRateLimitResetAtFromSnapshot(snapshot, baseTime)
		if len(updates) > 0 {
			if resp.StatusCode < 200 || resp.StatusCode >= 300 {
				return updates, resetAt, fmt.Errorf("openai codex probe returned status %d", resp.StatusCode)
			}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		if len(updates) > 0 {
			return updates, nil, fmt.Errorf("openai codex probe returned status %d", resp.StatusCode)
		}
