...
# Enable corepack only. The exact pnpm version is resolved from the
# "packageManager" field in frontend/package.json (copied in below), so CI,
# Docker and local dev all pin to one place instead of three.
RUN corepack enable
